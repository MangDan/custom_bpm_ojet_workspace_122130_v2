...
define(['ojs/ojcore', 'knockout', 'jquery'],
  function (oj, ko, $) {
    function CloseInstanceViewModel() {
      var self = this;
      ...
      instanceListByAjax = function (status, limit, offset) {
        ...
        $.ajax({
          type: "GET",
          url: obpmConfig.serverurl + obpmConfig.resturi + "processes?processState=COMPLETED&orderBy=processId:desc",
          cache: false,
          headers: {
      ...
      self.instanceAuditDialog = function (data, event) {
        ...
        var loadModuleConfig = { 'modules': [{ 'title':'Process Map', 'name': 'processTrackingIframe', 'params':{'processId':data.processId} }, 
      ...
      self.instanceAuditDialog2 = function (data, event) {
        ...
        // Module 공통화 (아래와 같이 세팅하고 실행하면 해당 모듈이 로드되는 구조.)
        var loadModuleConfig = { 'modules': [{ 'title':'Process Map', 'name': 'processTracking', 'params':{'processId':data.processId} }, 
                                          { 'title':'Activity History', 'name': 'activityHistory', 'params':{'processId':data.processId} }]
                              };
        self.loadTrackingModule(loadModuleConfig);
     ...
     */
    return new CloseInstanceViewModel();
  }
);
